...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func showMainWindowIfNeeded() {
        if mainWindowController == nil {
            ...
            mainWindowController = storyboard.instantiateController(
    ...
    private func handlePassRequest(requestID: String?) {
        ...
        do {
            ...
            try? writePassResponse(requestID: requestID, payload: [
                "requestId": requestID,
                "ok": false,
    ...
    private func acquireURLIndexRefreshLock() -> Bool {
        ...
        let didCreate = fileManager.createFile(atPath: lockFileURL.path, contents: Data(), attributes: [.posixPermissions: 0o600])
        return didCreate
    }
    ...
    private func storeInventory(in configuration: StoreConfiguration) throws -> StoreInventory {
        ...
        guard fileManager.fileExists(atPath: configuration.url.path, isDirectory: &isDirectory) else {
            throw PassError.executionFailed("Password store not found or is not accessible at \(configuration.url.path).")
        }
    ...
    private func resolvedOTPDetails(for entryName: String, configuration: StoreConfiguration) throws -> OTPDetails {
        let code = try runPass(arguments: ["otp", "code", entryName], configuration: configuration)
        let uriOutput = try? runPass(arguments: ["otp", "uri", entryName], configuration: configuration)
    ...
    private func otpMetadata(from rawURI: String?) -> (type: String?, period: Int?) {
        guard let trimmedURI = rawURI?.trimmingCharacters(in: .whitespacesAndNewlines),
              !trimmedURI.isEmpty,
              let components = URLComponents(string: trimmedURI) else {
            return (nil, nil)
        }
    ...
    private func normalizedHost(from rawHost: String?) -> String? {
        ...
        let hostWithoutPath = trimmedHost.split(separator: "/", maxSplits: 1, omittingEmptySubsequences: true).first.map(String.init) ?? trimmedHost
        return hostWithoutPath.hasPrefix("www.") ? String(hostWithoutPath.dropFirst(4)) : hostWithoutPath
    }
    ...
    private func runPass(arguments: [String], configuration: StoreConfiguration) throws -> String {
        ...
        guard process.terminationStatus == 0 else {
            let message = errorOutput.isEmpty ? "'pass' exited with status \(process.terminationStatus)." : errorOutput
